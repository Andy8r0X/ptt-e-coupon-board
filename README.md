...
2. 腳本讀取 `state.json` 中記錄的最後掃描頁碼與已知文章 ID，從 `INITIAL_START_PAGE` 開始向後抓取 PTT e-coupon 板的頁面，解析文章列表
3. 統計結果輸出為 `stats.json`，同時保留歷史檔於 `export/` 資料夾
4. `state.json` 會更新已抓取的文章 ID 與最後掃描頁碼，供下次增量更新使用
5. **GitHub Pages** 載入 `index.html`，透過 `js/main.js` 讀取 `stats.json` 顯示統計結果
